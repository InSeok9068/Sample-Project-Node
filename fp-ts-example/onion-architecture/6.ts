...
interface MonadApp<M extends URIS> extends MonadEmail1<M>, MonadFileSystem1<M>, Monad1<M>, Applicative1<M> {}
...
// pure
const sendGreetings =
  <M extends URIS>(M: MonadApp<M>) =>
  (fileName: string, today: Date): Kind<M, void> => {
    return M.map(
      M.chain(
