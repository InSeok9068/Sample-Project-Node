...
const brightness = (color: Color): number => (color.red + color.green + color.blue) / 3;
...
export const invert = (c: Color): Color => color(255 - c.red, 255 - c.green, 255 - c.red);

// if brightness over some value V then put White else put Black
export const threshold = (c: Color): Color => (brightness(c) < 100 ? BLACK : WHITE);
