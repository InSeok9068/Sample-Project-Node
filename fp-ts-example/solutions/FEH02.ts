...
const getMonoid = <A>(S: Semigroup<A>): Monoid<O.Option<A>> => ({
  concat: (first, second) =>
    O.isNone(first) ? second : O.isNone(second) ? first : O.some(S.concat(first.value, second.value)),
  empty: O.none,
});
...
assert.deepStrictEqual(concatAll(M)([O.some(1), O.some(2), O.none, O.some(3)]), O.some(6));
