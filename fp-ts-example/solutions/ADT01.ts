...
export const node = <A>(value: A, left: BinaryTree<A>, right: BinaryTree<A>): BinaryTree<A> => ({
  _tag: "Node",
  value,
  left,
  right,
...
export const fold =
  <A, B>(onLeaf: (a: A) => B, onNode: (value: A, left: BinaryTree<A>, right: BinaryTree<A>) => B) =>
  (tree: BinaryTree<A>): B => {
    switch (tree._tag) {
      case "Leaf":
...
export const toReadonlyArray: <A>(tree: BinaryTree<A>) => ReadonlyArray<A> = fold(
  (value) => [value],
  (value, left, right) => [value].concat(toReadonlyArray(left)).concat(toReadonlyArray(right)),
);
