...
const Monad: Monad2<URI> = {
  ...
  of: (a) => T.of(E.of(a)),
  ap: (fab, fa) => () => Promise.all([fab(), fa()]).then(([eab, ea]) => pipe(eab, E.ap(ea))),
...
async function test() {
  assert.deepStrictEqual(await Monad.map(Monad.of(1), (n: number) => n * 2)(), E.right(2));
  assert.deepStrictEqual(
    await Monad.chain(Monad.of(1), (n: number) => (n > 0 ? Monad.of(n * 2) : T.of(E.left("error"))))(),
    E.right(2),
  );
  assert.deepStrictEqual(
    await Monad.chain(Monad.of(-1), (n: number) => (n > 0 ? Monad.of(n * 2) : T.of(E.left("error"))))(),
